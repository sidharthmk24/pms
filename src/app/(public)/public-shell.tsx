...
export default function PublicShell({ children }: { children: React.ReactNode }) {
  ...
  // Distraction-free mode for focused contract signing and proof review documents
  const isDocumentFlow =
    pathname?.startsWith("/publish/contract") ||
    pathname?.startsWith("/publish/proof-approval");
  ...
  if (isDocumentFlow) {
    return (
      <div className="relative min-h-dvh flex flex-col bg-background selection:bg-primary selection:text-white">
        ...
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[500px] overflow-hidden opacity-60"
        >
          <div className="absolute left-1/2 top-[-140px] h-[500px] w-[900px] -translate-x-1/2 rounded-full bg-gradient-to-b from-[#7e2562]/[0.08] via-[#9b3179]/[0.02] to-transparent blur-3xl" />
        </div>
